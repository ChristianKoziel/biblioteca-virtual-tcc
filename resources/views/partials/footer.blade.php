<!-- Footer -->
<footer class="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-10 mt-8">
    <div class="container mx-auto px-4">
        <div class="flex flex-col md:flex-row justify-between items-center">
            <div class="mb-6 md:mb-0">
                <div class="text-2xl font-bold flex items-center">
                    <i class="fas fa-book-open mr-2"></i> Bookfy
                </div>
                <p class="text-gray-400 mt-2">Sua biblioteca digital</p>
            </div>
            
            <div class="flex space-x-6">
                {{-- <a href="#" class="text-gray-400 hover:text-white transition-all duration-200">
                    <i class="fab fa-facebook-f text-xl"></i>
                </a>
                <a href="#" class="text-gray-400 hover:text-white transition-all duration-200">
                    <i class="fab fa-twitter text-xl"></i>
                </a> --}}
                <a href="https://www.instagram.com/usebookfy?igsh=MWRsOThwdzZ6cGtqMA%3D%3D&utm_source=qr" class="text-gray-400 hover:text-white transition-all duration-200">
                    <i class="fab fa-instagram text-xl"></i>
                </a>
            </div>
        </div>
        
        <div class="border-t border-gray-700 mt-6 pt-6 text-center text-gray-400">
            <p>&copy; 2025 Bookfy. Todos os direitos reservados.</p>
        </div>
    </div>
</footer>   